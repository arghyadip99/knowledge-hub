...
export const knowledgeImportSchema = z.object({
  source: z.object({
    type: z.enum(sourceTypes).default('youtube'), title: z.string().min(3).max(300), url: z.string().url().optional().or(z.literal('')), creator: z.string().min(2).max(200), publisher: z.string().max(200).optional(),
    publishedAt: optionalDate, durationSeconds: z.number().int().positive().optional(), language: z.string().max(20).default('en'), focusArea: flexibleFocusArea.default('General'), rawTranscript: z.string().max(500000).optional()
  }),
  knowledge: z.object({
    centralThesis: z.string().min(20).max(2000), summary: z.string().min(50).max(8000), whyItMattersToMe: z.string().max(2000).optional(), tags: z.array(z.string().min(1).max(50)).max(15).default([]), status: z.enum(['inbox', 'distilled', 'applied']).default('distilled'), nextReviewAt: optionalDate
  }),
